...
def test_moe_align_block_size(
    ...
    print(2)
    # 临时调试信息：放在这里
    diff_mask = actual_expert_ids != golden_expert_ids
    if diff_mask.any():
        ...
        print("expert_ids length:", actual_expert_ids.numel())
        print("different indices:", diff_indices.cpu())
        print("actual different values:",
            actual_expert_ids[diff_mask].unique().cpu())
        print("golden different values:",
            golden_expert_ids[diff_mask].unique().cpu())

        start = max(0, first_diff - 8)
        end = min(actual_expert_ids.numel(), first_diff + 40)
        print(f"comparison range [{start}:{end}]")
        print("actual:", actual_expert_ids[start:end].cpu())
        print("golden:", golden_expert_ids[start:end].cpu())


    print(3)
    # torch.testing.assert_close(actual_expert_ids, golden_expert_ids, atol=0, rtol=0)

    actual_num_blocks = cdiv(
        int(actual_num_tokens.item()),
        block_size,
    )

    # 只比较会被后续 MoE GEMM 实际使用的 block。
    torch.testing.assert_close(
        actual_expert_ids[:actual_num_blocks],
        golden_expert_ids[:actual_num_blocks],
        atol=0,
        rtol=0,
    )
